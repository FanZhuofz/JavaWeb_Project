...
public class NewsTypeServlet extends HttpServlet {
	...
	private void newsTypePreSave(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{
		Connection con=null;
	...
	private void newsTypeBackList(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{
		...
		try{
			...
			request.setAttribute("mainPage", "newsType/newsTypeList.jsp");
			request.getRequestDispatcher("background/mainTemp.jsp").forward(request, response);
		}catch(Exception e){
			e.printStackTrace();
		}finally{
	...
	private void newsTypeDelete(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException{
		...
		try{
			...
			if(exist){
				result.put("errorMsg", "该新闻类别下有新闻，不能删除此类别！");
			}else{
